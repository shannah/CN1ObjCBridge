...
import com.codename1.objc.mappers.NSObjectMapping;
import com.codename1.objc.mappers.PointerMapping;
import com.codename1.objc.mappers.ScalarMapping;
import com.codename1.objc.mappers.StringMapping;
import com.codename1.objc.mappers.StructureMapping;
...
public class TypeMapper implements TypeMapping {
    ...
    private void init(){
        ...
        String chars = "cCiIsSfdlLqQB[:b?#v";
        String[] args = new String[chars.length()];
        for (int i=0; i<args.length; i++) {
            args[i] = String.valueOf(chars.charAt(i));
        }
        addMapping(new ScalarMapping(), args);
        addMapping(new StringMapping(), "*");
    ...
    public Object jToC(Object jVar, String signature, TypeMapping root) {
        ...
        if (signature.length() > 0) {
            ...
            Log.p("Warning: Empty signature provided to jToC at: ");
            Log.e(new RuntimeException("here"));
            throw new RuntimeException("Invalid signature provided");
        }
        if ( offset > 0 ){
            signature = signature.substring(offset);
        }
